...
fn palindrome_min_edit(s:&str, k:usize) -> usize {
    ...
    let mut distances = vec![vec![100;n];n];
    let mut memory = vec![vec![usize::MAX;k+1];n];
    for i in 0..n {
        for j in i..n {
            distances[i][j] = palindrome_distance(s_bytes, i, j);
        }
    }
    dfs_edit(s, 0, k, &distances, &mut memory)
...
fn dfs_edit(
    ...
    if memo[start][k] != usize::MAX {
        return memo[start][k];
    }
    let mut min_cost = usize::MAX;
    for end in start+1..=s.len() {
        let curr = dist[start][end-1];
        let rest = dfs_edit(s, end, k-1, dist, memo);
        if rest < usize::MAX {
            min_cost = min_cost.min(curr + rest);
        }
    }
    memo[start][k] = min_cost; 
    min_cost
}


fn palindrome_distance(s:&[u8], i:usize, j:usize) -> usize {
    let mut dist = 0;
    for idx in 0..(j - i + 1)/2 {
        if s[idx + i] != s[j - idx] {
            dist += 1;
        }
    }
...
fn main() {
    // println!("partitioning {:?}", palindrome_min_edit("abcd", 2));
    println!("partitioning {:?}", palindrome_min_edit("aabbcb", 3));
    println!("partitioning {:?}", palindrome_min_edit("aabbc", 3));
    // println!("partitioning {:?}", palindrome_min_edit("aabbcbc", 3));
