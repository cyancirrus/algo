fn sum_xor_dp(nums:&[usize]) -> usize {
    ...
    for j in 0usize..1<<n {
        let lowest = j & (!j + 1);
        let pos = lowest.trailing_zeros() as usize;
        memo[j] ^= memo[j ^ lowest] ^ nums[pos];
        sum += memo[j];
    }
    sum
}
...
fn sum_xor(nums:&[usize]) -> usize {
    _sum_xor_(0, 0, nums)
}

fn _sum_xor_(c_sum:usize, idx:usize, nums:&[usize]) -> usize {
    if idx == nums.len()  { return c_sum };
    _sum_xor_(c_sum ^ nums[idx], idx + 1, nums) + _sum_xor_(c_sum, idx + 1, nums)
}


// fn sum_xor(nums:&[usize]) -> usize {
//     // cleaner for contiguous elements
//     let mut total = 0;
//     for start in 0..nums.len() {
//         let mut ss_total =  0;
//         // for all running subsets
...
// fn sum_xor(nums:&[usize]) -> usize {
//     // this is for contiguous elements
//     if nums.is_empty() { return 0 };
//     let mut total = 0;
//     let mut ss_total =  0;
//     for &num in nums {
//         // for all running subsets
//         ss_total ^= num;
//         // add them into the total
//         total += ss_total;
//     }
//     // compute subsets for the next elements
//     total += sum_xor(&nums[1..]);
//     total
// }
...
fn main() {
    binary_watch(1);
    println!("Sum xor subset {}", sum_xor(&[5,1,6]));
