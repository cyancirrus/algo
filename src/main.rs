fn game_of_life_precomp(grid:&mut Vec<Vec<u8>>) -> &Vec<Vec<u8>> {
    if grid.is_empty() || grid[0].is_empty() { return grid; }
    let m = grid.len();
    let n = grid[0].len();
    let mut horiz = vec![vec![0;n];m];
    for x in 0..m {
        let mut hsum = grid[x][0];
        for y in 0..n {
            let ny = y + 1;
            if ny < n { hsum += grid[x][ny]; }
    ...
    for y in 0..n {
        ...
        for x in 0..m {
            let nx = x + 1;
            if nx < m { vsum += horiz[nx][y]; }
            let nx = x.wrapping_add(!1);
            if nx < m { vsum -= horiz[nx][y]; }
            verticals[x] = vsum;
        }
        for x in 0..m {
            let neighbors = verticals[x] - grid[x][y];
