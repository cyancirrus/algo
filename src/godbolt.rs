...
//
// investigating compiled code on godbolt.com
...
fn main() {
    ...
    for _ in 0..1_000_000 {
        for n in 70..=90 {
            // Prevent optimization of both input and output
            let input = black_box(n);
            let res = black_box(run_fib(input));
            sum = sum.wrapping_add(res);
        }
    }

    let duration = start.elapsed();
    println!("fib(70..=90) x 1_000_000 in {:?}", duration);
    // Do something with `sum` so compiler doesn’t discard it
    black_box(sum);
}
...
//     }
//     let duration = start.elapsed();
//     println!("fib()  (1_000_000x) in {:?}", duration);
// }
...
//     }
//     let duration = start.elapsed();
//     println!("Run fib n, t: ({}, {:?})", res, duration);
// }
